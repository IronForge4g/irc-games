...
class phaseCemetary {
  ...
  function init() {
    $this->r->mChan($this->r->currentPlayer->nick.": You have ended up at the Cemetary. Please !draw a card, or !pass this action.");
    $this->card = null;
  }
  function cmddraw($from, $args) {
    if(!($this->r->checkCurrentPlayer($from, 'Cemetary Card'))) return;
    if($this->card != null) {
      $this->r->mChan($from.": A card has already been drawn.");
      return;
    }
    $this->card = $this->r->cemetaryDeck->draw();
