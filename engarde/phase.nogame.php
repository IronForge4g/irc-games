...
class phaseEnGardeNoGame {
  ...
  function cmdjoin($from, $args) {
    ...
    $this->r->players[$from] = new enGardePlayer($this->r, $from);
  ...
  function cmdstart($from, $args) {
    ...
    if(isset($args[0])) {
      if(is_numeric($args[0]) && $args[0] > 0 and $args[0] < 10) {
        $this->r->score = $args[0];
      } else {
        $this->r->mChan("$from: End game point must be 1-9.");
        return; 
      }
    }
    else $this->r->score = 5;
